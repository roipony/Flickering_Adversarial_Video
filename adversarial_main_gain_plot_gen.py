...
def read_mp4_file(item):
   
    frames = skvideo.io.vread(item.decode("utf-8"))
...
list_train = [x.strip() for x in open('/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/train_path_list_npy.txt')]
random.shuffle(list_train)
list_train=list_train[:train_size]
train_image_paths = tf.convert_to_tensor(list_train, dtype=tf.string)


dataset_train = tf.data.Dataset.from_tensor_slices(train_image_paths)

dataset_train=dataset_train.map(
        lambda item: tuple(tf.py_func(read_npy_file, [item], [tf.float32]) ) ,  num_parallel_calls=30).batch(_BATCH_SIZE, drop_remainder=True)

dataset_train = dataset_train.prefetch(_BATCH_SIZE*4)

iterator_train = dataset_train.make_initializable_iterator()
next_element_train = iterator_train.get_next()


list_val = [x.strip() for x in open('/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/val_path_list_npy.txt')]
random.shuffle(list_val)
list_val=list_val[:val_size]

val_image_paths = tf.convert_to_tensor(list_val, dtype=tf.string)


dataset_val = tf.data.Dataset.from_tensor_slices(val_image_paths)

dataset_val=dataset_val.map(
        lambda item: tuple(tf.py_func(read_npy_file, [item], [tf.float32]) ),num_parallel_calls=30 ).batch(_BATCH_SIZE, drop_remainder=True)

dataset_val = dataset_val.prefetch(_BATCH_SIZE*4)
...
while True:
    ...
    try:
        ...
        rgb_sample,  = sess.run(next_element_train)
