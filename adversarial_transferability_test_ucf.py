...
ucf_classes = [x.strip().lower() for x in open(_LABEL_MAP_PATH_UCF_101)]
...
# adversarial_inputs_rgb = tf.clip_by_value(cyclic_flag*cyclic_rgb_input +(1-cyclic_flag)*rgb_input + adv_flag * (mask * extend_eps_rgb),
#                                           clip_value_min = -1.0,
#                                           clip_value_max = 1.0,
...
sess =tf.Session() 
mask_np = np.ones(shape=[90,224,224,3],dtype=np.float32)
# mask_np[:,56:56+122,56:56+122,:]=0.0
mask = tf.Variable(tf.ones_like(mask, dtype=tf.float32))
# mask = tf.cast(mask,dtype=tf.float32)
oo=mask.assign(mask_np)
sess.run(oo)
...
latest_ckp = tf.train.latest_checkpoint('result/generalization/single_class/pull ups/')
latest_ckp = tf.train.latest_checkpoint('/data/DL/Adversarial/kinetics-i3d/result/generalization/model_gen_untargeted_ce_loss_reg/')

reader = pywrap_tensorflow.NewCheckpointReader(latest_ckp)
kinetiks_pert = reader.get_tensor('RGB/eps')
ass_op=eps_rgb.assign(kinetiks_pert)
sess.run(ass_op)

vid_path = '/data/DL/Adversarial/database/UCF-101/video/v_PullUps_g08_c02.avi'
vid_class_name = vid_path.split('/')[-1].split('_')[1]
vid_class_id = ucf_classes.index(vid_class_name.lower())
rgb_sample = img_tool.video_to_image_and_of(vid_path,target_fps=25,
                                            n_steps=_SAMPLE_VIDEO_FRAMES)
feed_dict[rgb_input] = rgb_sample[0]
feed_dict[adv_flag] = True
feed_dict[cyclic_flag] = True

feed_dict_clean[rgb_input] = rgb_sample[0]
feed_dict_clean[adv_flag] = False


res_per=sess.run(model_predictions,feed_dict)
res_clean =sess.run(model_predictions,feed_dict_clean)

print('video GT: {}[{}] \ni3d clean: {} {}[{}] \ni3d pert: {} {}[{}]'.format(vid_class_name, vid_class_id, 
                                                        res_clean[0].max(), ucf_classes[res_clean[0].argmax()],
                                                        res_clean[0].argmax(),res_per[0].max(), ucf_classes[res_per[0].argmax()],
                                                        res_per[0].argmax()))
#%%
fooling = 0
N_vid = 0
video_path ='/data/DL/Adversarial/database/UCF-101/video/'
video_list = glob.glob(video_path + '/*.avi')
for v in video_list:
    vid_class_name = v.split('/')[-1].split('_')[1]
    vid_class_id = ucf_classes.index(vid_class_name.lower())
    rgb_sample = img_tool.video_to_image_and_of(v,target_fps=25,
                                        n_steps=_SAMPLE_VIDEO_FRAMES)
    if rgb_sample[0].shape[1]<90:
        continue
    ...
    if res_clean[0].argmax() != vid_class_id:
        continue
    
    res_per=sess.run(model_predictions,feed_dict)
    N_vid+=1
    
    if res_per[0].argmax() != vid_class_id:
        fooling +=1
    
    print('total videos: {}, fooling ratio: {}'.format(N_vid,fooling/N_vid))
