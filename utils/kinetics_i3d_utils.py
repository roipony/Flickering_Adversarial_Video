...
def init_model(model,sess, ckpt_path, eval_type='rgb',extra_variable=None):
    rgb_variable_map = {}
    ...
    for variable in model.graph.get_collection_ref('moving_average_variables'):
      ...
      if variable.name.split('/')[0] == 'RGB':
        if eval_type == 'rgb600':
          ...
          rgb_variable_map[variable.name.replace(':0', '')] = variable
    if extra_variable:
        for v in extra_variable:
            rgb_variable_map[v.name.replace(':0', '')]=v
...
class kinetics_i3d():
  ...
  def __init__(self, ckpt_path='data/checkpoints/rgb_imagenet/model.ckpt',
    ...
    self.init_pert_from_ckpt=init_pert_from_ckpt
    scope ='RGB'
    ...
    with tf.variable_scope(scope):
        ...
        self.eps_rgb_clip = tf.clip_by_value(self.eps_rgb,clip_value_min = -1.*(self.inf_norm*2),
                                              clip_value_max = 1.*(self.inf_norm*2))
  ...
  def init_model(self):
      ...
      init_model(model=self.rgb_model,
                 sess=self.sess,
                 ckpt_path=self.ckpt_path,
                 eval_type='rgb',
                 extra_variable= [self.perturbation] if self.init_pert_from_ckpt else None )
