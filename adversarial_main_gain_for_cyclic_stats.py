...
result_path = 'result/batch_cyclic/'
save_result =True
stats = []
...
tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[0:5]

# tf_record_list_train = [x.strip() for x in open(tf_records_list_path_train)]
dataset_val = tf.data.TFRecordDataset(filenames=tf_record_list_val, num_parallel_reads=tf_record_list_val.__len__())
dataset_val = dataset_val.prefetch(buffer_size=_BATCH_SIZE*5)
dataset_val = dataset_val.batch(_BATCH_SIZE,drop_remainder=True)
dataset_val = dataset_val.map(img_tool.parse_example_uint8, num_parallel_calls=1)
...
#%%
        while True:  # to repeat with decreased epsilons if necessary
            ...
            print(
                ...
                ", top_prob: {:.5f}, fatness: {:.5f} ({:.2f} %), smoothness: {:.5f} ({:.2f} %)".format(step, total_loss[0],
                                                                                                       adv_loss[0],
                                                                                                       reg_loss,
                                                                                                       norm_reg_loss,
                                                                                                       diff_norm_reg_loss,
