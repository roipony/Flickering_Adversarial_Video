...
def generate_input_fn(ATTACK_CFG, mode=tf.estimator.ModeKeys.EVAL, batch_size=1):
    ...
    #         tf_record_list += sorted(glob.glob(p + '/*.tfrecords'))
...
if sys.argv.__len__()>1:
    ...
    cfg = ki3du.load_config(yml_path='run_config_L12.yml')
...
model_dir = os.path.join(ATTACK_CFG.PKL_RESULT_PATH,'{}_t{}_v{}_'.format(source_class,
...
destribution = tf.distribute.MirroredStrategy(cross_device_ops=tf.distribute.HierarchicalCopyAllReduce()) #,devices=["/gpu:2","/gpu:3"]) #(devices=["/gpu:0"])#(devices=["/gpu:0"]) #devices=["/gpu:0", "/gpu:1"]
...
if 0:
    ...
    tf_record_list_val =tf_record_list_val[0:10]
    # tf_record_list_train = tf_record_list_train[:ATTACK_CFG.NUM_OF_TRAIN_TF_RECORDS]
