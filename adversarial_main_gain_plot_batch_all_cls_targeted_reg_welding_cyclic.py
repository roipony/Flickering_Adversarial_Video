...
# regularization loss:
_beta_0 =100.0 # 10.0 #0.1# 1 #1 #1.0 #1.0
...
tf_record_list_train = sorted(glob.glob(tf_records_list_path_train+ '*.tfrecords'))[0:25]
...
tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[-20:-1]
...
target_class = 'welding' # 'javelin throw'
target_class_id = kinetics_classes.index(target_class)




ckpt_dst = 'train_result/model_gen_targeted_ce_loss_reg_welding_cyclic/'
...
if 0:
    tf_records_list_path_val = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord_uint8/val/'
    tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[-5:-1]
...
while True:
    ...
    try:
        ...
        try: 
            ...
            val_step_l.append(step)
            print("step: {:05d} ,fool_rate: {:.5f}".format(step, miss_rate/total_val_vid) )
