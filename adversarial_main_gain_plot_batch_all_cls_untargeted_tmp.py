...
_SAMPLE_VIDEO_FRAMES =250 #90 #90 #79 # 79 #90 #90 #250 #90 #79
...
# l_3 =max_non_correct_cls -(corret_cls_prob-margin)



beta_default = tf.constant(1, dtype=tf.float32)
beta_ = tf.placeholder_with_default(beta_default, name='beta_', shape=beta_default.shape)
...
ce_loss_mean = -1.0*beta_*tf.reduce_sum( ce_loss)
...
# regularization loss:
_beta_0 =0 # 10.0 #0.1# 1 #1 #1.0 #1.0
...
tf_records_list_path_train = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord_250/val/'
tf_record_list_train = sorted(glob.glob(tf_records_list_path_train+ '*.tfrecords'))[0]
...
dataset_train = dataset_train.prefetch(buffer_size=_BATCH_SIZE*4)
dataset_train=dataset_train.repeat(100)
iterator_train = dataset_train.make_initializable_iterator()
next_element_train = iterator_train.get_next()

tf_records_list_path_val = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord_250/val/'
tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[0]
# tf_record_list_val=tf_record_list_val[:10]
# tf_records_list_path_val = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord/val_tfrecords.txt'
...
dataset_val = dataset_val.prefetch(buffer_size=_BATCH_SIZE*4)
# dataset_val=dataset_val.repeat(100)
...
target_class = 'jogging' # 'javelin throw'
...
# ckpt_last = tf.train.latest_checkpoint(checkpoint_dir='train_result/model_gen_targeted/')
# if ckpt_last:
...
try: 
    ...
    while True:
        ...
        miss_rate+=(prob.argmax(axis=-1)==sample_label).sum()
        total_val_vid+=rgb_sample.shape[0]
...
# saver.save(sess,'train_result/model_gen_targeted/model_step_{:05d}'.format(step))
...
while True:
    ...
    try:
        ...
        feed_dict_for_train = {inputs: rgb_sample,
                           labels:sample_label ,#[target_class_id]*_BATCH_SIZE,
        ...
        start = time.perf_counter()
        _, prob,total_loss, adv_loss, reg_loss, norm_reg_loss, diff_norm_reg_loss, _laplacian_norm_reg, _fatness, _smoothness = \
        sess.run(fetches=[train_op,softmax, loss, ce_loss_mean, regularizer_loss, norm_reg,diff_norm_reg, laplacian_norm_reg, fatness, smoothness],
        ...
        print(
            "Step: {:05d}, Total Loss: {:.5f}, Cls Loss: {:.5f}, Total Reg Loss: {:.5f}, Fat Loss: {:.5f}, Diff Loss: {:.5f}fatness: {:.5f} ({:.2f} %), smoothness: {:.5f} ({:.2f} %), target_cls: {:.5f}".format(step, total_loss,
                                                                                                    adv_loss,
                                                                                                    reg_loss,
                                                                                                    norm_reg_loss,
                                                                                                    diff_norm_reg_loss,
                                                                                                    ...
                                                                                                    _fatness / 2.0 * 100,
                                                                                                    _smoothness,
                                                                                                    _smoothness / 2.0 * 100,prob[:,sample_label].mean()))
        ...
        try: 
            # Keep running next_batch till the Dataset is exhausted
            while True:
                rgb_sample = sess.run(next_element_val)[0]
                
                # rgb_sample=rgb_sample[:,-_SAMPLE_VIDEO_FRAMES:,...]
                feed_dict_for_adv_eval = {inputs: rgb_sample, adv_flag: 1}
                prob = sess.run(feed_dict=feed_dict_for_adv_eval, fetches=softmax)
                miss_rate+=(prob.argmax(axis=-1)==target_class_id).sum()
                total_val_vid+=rgb_sample.shape[0]
        ...
        # with open('train_result/model_gen_targeted/res.pkl', 'wb') as file:
        ...
        # tf_records_list_path_train = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord/train/'
        # tf_record_list_train = glob.glob(tf_records_list_path_train+ '*.tfrecord')
        # # tf_record_list_train = [x.strip() for x in open(tf_records_list_path_train)]
        # dataset_train = tf.data.TFRecordDataset(filenames=tf_record_list_train, num_parallel_reads=10)
        ...
        # tf_records_list_path_val = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord/val/'
        # tf_records_list_path_val = glob.glob(tf_records_list_path_val+ '*.tfrecord')
