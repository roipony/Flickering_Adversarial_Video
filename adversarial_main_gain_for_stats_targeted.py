...
_BASE_PATCH_FRAMES =_SAMPLE_VIDEO_FRAMES# _SAMPLE_VIDEO_FRAMES #_SAMPLE_VIDEO_FRAMES #_SAMPLE_VIDEO_FRAMES # 1# _SAMPLE_VIDEO_FRAMES # 1:for sticker _SAMPLE_VIDEO_FRAMES # 1
_IND_START = 0  # 0 #50
...
tgt_cls_prob = tf.reduce_sum(tf.gather(softmax,labels,axis=-1))
tgt_cls_logits = tf.reduce_sum(tf.gather(model_logits,labels,axis=-1))
...
_is_adversarial = tf.cast(tf.reduce_all(tf.equal(tf.argmax(softmax,axis=-1),labels)), dtype=tf.float32)
...
max_non_tgt_cls = tf.reduce_max(softmax-tf.one_hot(labels,NUM_CLASSES),axis=-1)
max_non_tgt_cls_logits = tf.reduce_max(model_logits-tf.one_hot(labels,NUM_CLASSES),axis=-1)


margin =0.05
loss_margin= tf.log(1.+ margin*(1./tgt_cls_prob))

l_1= 0.0
l_2 = ((max_non_tgt_cls_logits-(tgt_cls_logits-loss_margin))**2)/loss_margin
l_3 = max_non_tgt_cls_logits-(tgt_cls_logits-loss_margin)
...
# regularization loss:
_beta_0 =1.0# 1.0 #0.1# 1 #1 #1.0 #1.0
...
result_path = 'result/single_video_attack/targeted/'
save_result =True
stats = []
...
index = setGPU.bestGPU
tf_records_list_path_val = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord_uint8/val/'
tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '/*/*.tfrecords'))[index*10:index*10+10]
#import pdb
#pdb.set_trace()
...
dataset_val = dataset_val.shuffle(buffer_size=_BATCH_SIZE*5)
...
while True:
    ...
    # continue
        ...
        if top_id!=sample_label or top_id_prob>0.99:
        ...
        while True:
            random_cls =  random.choice(kinetics_classes)
            if random_cls != correct_cls:
                target_class = random_cls  # 'javelin throw'
...
#%%
        while True:  # to repeat with decreased epsilons if necessary
            ...
            sess.run(fetches=[train_op, loss, ce_loss_mean, regularizer_loss, norm_reg,diff_norm_reg, laplacian_norm_reg, fatness, smoothness,max_non_tgt_cls],
            ...
            print(
                ...
                ", target_prob: {:.5f}, fatness: {:.5f} ({:.2f} %), smoothness: {:.5f} ({:.2f} %)".format(step, total_loss[0],
                                                                                                       adv_loss[0],
                                                                                                       reg_loss,
                                                                                                       norm_reg_loss,
                                                                                                       diff_norm_reg_loss,
