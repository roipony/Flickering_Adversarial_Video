...
model_dir = os.path.join(ATTACK_CFG.PKL_RESULT_PATH,source_class)
if not os.path.exists(model_dir):
    os.makedirs(model_dir)
    
#%%

destribution = tf.distribute.MirroredStrategy(cross_device_ops=tf.distribute.HierarchicalCopyAllReduce()) #(devices=["/gpu:0"])#(devices=["/gpu:0"]) #devices=["/gpu:0", "/gpu:1"]
run_config = tf.estimator.RunConfig(
               model_dir=model_dir,
               tf_random_seed=None,
               save_summary_steps=10,
               save_checkpoints_steps=17,
...
regex = '^(?!.*global_step).*$' #without global_step
#regex = '.*' # all var include non-trainble

warm_start_from=tf.estimator.WarmStartSettings(
        ckpt_to_initialize_from=str(cfg.MODEL.CKPT_PATH_WITH_ZERO_PERT),vars_to_warm_start=[regex])
...
profile_hook = tf.train.ProfilerHook(save_steps=10, output_dir='.')
