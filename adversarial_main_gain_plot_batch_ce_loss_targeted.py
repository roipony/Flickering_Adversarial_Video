...
tf_records_list_path_train = '/data/DL/Adversarial/kinetics-i3d/data/tfrecords/triple_jump_90_frames/train/'
tf_record_list_train = sorted(glob.glob(tf_records_list_path_train+ '*.tfrecords'))[0:12]
...
dataset_train = dataset_train.map(img_tool.parse_example_batch, num_parallel_calls=os.cpu_count())
# dataset_train = dataset_train.apply(tf.contrib.data.shuffle_and_repeat(buffer_size=10000))
    # dataset = dataset.shuffle(buffer_size=buffer_size)
...
tf_records_list_path_val ='/data/DL/Adversarial/kinetics-i3d/data/tfrecords/triple_jump_90_frames/val/'
tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))
...
dataset_val = dataset_val.map(img_tool.parse_example_batch, num_parallel_calls=os.cpu_count())
# dataset_train = dataset_train.apply(tf.contrib.data.shuffle_and_repeat(buffer_size=10000))
    # dataset = dataset.shuffle(buffer_size=buffer_size)
...
ckpt_dst = 'train_result/model_gen_one_class_targeted_ce_loss_reg/'
