...
tf_record_list_train = sorted(glob.glob(tf_records_list_path_train+ '*.tfrecords'))[0:100]
...
tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[-20:-80]
...
ckpt_dst = 'train_result/model_gen_targeted_ce_loss_reg_cyclic/'
...
pert = sess.run(eps_rgb)
pert_clip = pert/pert.__abs__().max()
try: 
    ...
    while True:
        ...
        feed_dict_for_adv_eval = {inputs: p_rgb_sample, adv_flag: 0, cyclic_flag:0}
        prob = sess.run(feed_dict=feed_dict_for_adv_eval, fetches=softmax)
        prob_clean = sess.run(feed_dict= {inputs: rgb_sample, adv_flag: 0, cyclic_flag:0}, fetches=softmax)
        miss_rate+=(np.logical_and(prob.argmax(axis=-1)!=sample_label,prob_clean.argmax(axis=-1)==sample_label)).sum()
