...
beta_4_default = tf.constant(0.1, dtype=tf.float32)
beta_4= tf.placeholder_with_default(beta_4_default, name='beta_4', shape=beta_4_default.shape)
...
gray_pert = 0.2989 * perturbation[...,0] + 0.5870 * perturbation[...,1]  + 0.1140 * perturbation[...,2] 

gray_norm = tf.reduce_sum(tf.abs(gray_pert))
...
regularizer_loss = beta_1*norm_reg + beta_2*diff_norm_reg +beta_3*laplacian_norm_reg +beta_4*gray_norm# +lab_reg
...
result_path = 'result/batch_rgb_2_gray/'
save_result =True
stats = []
...
tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[1:5]
...
while True:
    ...
    # continue
        ...
        if save_result:
            dict_result_path = os.path.join(result_path, 'res_{}_{:05d}'.format(gt_label_name.replace(' ','_'),k) + '.pkl')
...
#%%
        while True:  # to repeat with decreased epsilons if necessary
            ...
            _, total_loss, adv_loss, reg_loss, norm_reg_loss, diff_norm_reg_loss, _laplacian_norm_reg, _fatness, _smoothness,_max_non_correct_cls,_gray_norm = \
            sess.run(fetches=[train_op, loss, ce_loss_mean, regularizer_loss, norm_reg,diff_norm_reg, laplacian_norm_reg, fatness, smoothness,max_non_correct_cls, gray_norm],
            ...
            print(
                ...
                ", top_prob: {:.5f}, fatness: {:.5f} ({:.2f} %), smoothness: {:.5f} ({:.2f} %), gray_norm: {:.2f}".format(step, total_loss[0],
                                                                                                       adv_loss[0],
                                                                                                       reg_loss,
                                                                                                       norm_reg_loss,
                                                                                                       diff_norm_reg_loss,
            ...
            norm_gray_loss_l.append(_gray_norm)
