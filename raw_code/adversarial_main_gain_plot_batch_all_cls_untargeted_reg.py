...
with tf.variable_scope(scope):
    ...
    extend_eps_rgb = tf.clip_by_value(eps_rgb,clip_value_min = -0.5,
                                              clip_value_max = 0.5)
...
ce_loss = -1.0*tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=model_logits)
...
# regularization loss:
_beta_0 =10.0 # 10.0 #0.1# 1 #1 #1.0 #1.0
...
ckpt_dst = 'train_result/model_gen_untargeted_ce_loss_reg/'
