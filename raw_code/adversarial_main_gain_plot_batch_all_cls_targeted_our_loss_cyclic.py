...
# regularization loss:
_beta_0 =1.0 # 10.0 #0.1# 1 #1 #1.0 #1.0
...
tf_record_list_train = sorted(glob.glob(tf_records_list_path_train+ '*.tfrecords'))[0:15]
...
tf_records_list_path_val = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord_uint8/val/'
tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[15:30]
...
# tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[301:401]
...
# dataset_val = dataset_val.prefetch(buffer_size=_BATCH_SIZE*5)
# dataset_val = dataset_val.batch(_BATCH_SIZE,drop_remainder=True)
# dataset_val = dataset_val.map(img_tool.parse_example, num_parallel_calls=os.cpu_count())
# dataset_train = dataset_train.apply(tf.contrib.data.shuffle_and_repeat(buffer_size=10000))
#     dataset = dataset.shuffle(buffer_size=buffer_size)
...
# tf_records_list_path_train = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord/val/'
# tf_record_list_train = sorted(glob.glob(tf_records_list_path_train+ '*.tfrecords'))[0:28]

# # tf_record_list_train = [x.strip() for x in open(tf_records_list_path_train)]
# dataset_train = tf.data.TFRecordDataset(filenames=tf_record_list_train, num_parallel_reads=30)
...
# # Transformation
# dataset_train = dataset_train.apply(tf.contrib.data.map_and_batch(img_tool.parse_example,
...
# tf_records_list_path_val = '/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/tfrecord/val/'
# tf_record_list_val = sorted(glob.glob(tf_records_list_path_val+ '*.tfrecords'))[301:400]
# tf_record_list_val=tf_record_list_val[:10]
# # tf_records_list_path_val = '/data/DL/Adversarial/ActivityNet/Crawler/Kirnetics/database/tfrecord/val_tfrecords.txt'
# # tf_record_list_val = [x.strip() for x in open(tf_records_list_path_val)]
# dataset_val = tf.data.TFRecordDataset(filenames=tf_record_list_val, num_parallel_reads=os.cpu_count())
...
# # Transformation
# dataset_val = dataset_val.apply(tf.contrib.data.map_and_batch(img_tool.parse_example, batch_size=_BATCH_SIZE,drop_remainder=True,num_parallel_calls=os.cpu_count()))
...
target_class = 'juggling balls' # 'javelin throw'
...
ckpt_dst = 'train_result/model_gen_targeted_our_loss_reg_cyclic/'

if not os.path.exists(ckpt_dst):
    os.makedirs(ckpt_dst)
...
while True:
    ...
    try:
        ...
        feed_dict_for_train = {inputs: rgb_sample,
                           labels:[target_class_id]*_BATCH_SIZE,
        ...
        print(
            "Step: {:05d}, Total Loss: {:.5f}, Cls Loss: {:.5f}, Total Reg Loss: {:.5f}, Fat Loss: {:.5f}, Diff Loss: {:.5f}fatness: {:.5f} ({:.2f} %), smoothness: {:.5f} ({:.2f} %, target_prob: {:.5f})".format(step, total_loss,
                                                                                                    adv_loss,
                                                                                                    reg_loss,
                                                                                                    norm_reg_loss,
                                                                                                    diff_norm_reg_loss,
                                                                                                    ...
                                                                                                    _fatness / 2.0 * 100,
                                                                                                    _smoothness,
                                                                                                    _smoothness / 2.0 * 100,prob[:,target_class_id].mean()))
