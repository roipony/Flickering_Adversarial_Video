...
corret_cls_logits = tf.reduce_sum(tf.gather(model_logits,labels,axis=-1))
...
logits_scores, logits_ind = tf.math.top_k(tf.squeeze(model_logits,axis=0),k=2)

max_non_correct_cls_logits = scores[tf.cast(tf.squeeze(tf.equal(tf.cast(logits_ind[0], dtype=tf.int64), labels)),dtype=tf.int64)]
...
l_1= 0.0
l_2 = ((corret_cls_logits-(max_non_correct_cls_logits-margin))**2)/margin
l_3 = corret_cls_logits-(max_non_correct_cls_logits-margin)
...
for video in os.listdir(npy_path):
    ...
    if save_result:
        dict_result_path = os.path.join(result_path, video.split('.')[0]+'beta_1_{:.2f}_logits'.format(_beta_1) + '.pkl')
...
#%%
    while True:  # to repeat with decreased epsilons if necessary
        ...
        print(
            ...
            ", top_prob: {:.5f}, fatness: {:.5f} ({:.2f} %), smoothness: {:.5f} ({:.2f} %)".format(step, total_loss,
                                                                                                   adv_loss,
                                                                                                   reg_loss,
                                                                                                   norm_reg_loss,
                                                                                                   diff_norm_reg_loss,
