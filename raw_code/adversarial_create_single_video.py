...
_SAMPLE_VIDEO_FRAMES = 90 #90 #79
_BASE_PATCH_FRAMES = 1
_BATCH_SIZE = 1
_SAMPLE_PATHS = {
...
def main(unused_argv):
  ...
  if eval_type in ['rgb', 'rgb600', 'joint']:
    ...
    eps_rgb = tf.Variable(tf.zeros(shape=[_BASE_PATCH_FRAMES, _IMAGE_SIZE, _IMAGE_SIZE, 3], dtype=tf.float32))
    ...
    ind_start =0.0 #40.0 #ind#0 #50
    ind_end =_SAMPLE_VIDEO_FRAMES # 40.0 #_SAMPLE_VIDEO_FRAMES #ind #_SAMPLE_VIDEO_FRAMES #_SAMPLE_VIDEO_FRAMES# 50 #_SAMPLE_VIDEO_FRAMES # _SAMPLE_VIDEO_FRAMES

    default_T = tf.constant(int(ind_end-ind_start+1), dtype=tf.int32)
    T = tf.placeholder_with_default(default_T,
                                           shape=default_T.shape)

    # indices = tf.cast(tf.linspace(ind_start,ind_end,T), tf.int32)
  ...
  with tf.Session() as sess:
    ...
    if eval_type in ['rgb', 'rgb600', 'joint']:
      ...
      rgb_adv_model = fb_2.models.TensorFlowModel(inputs=rgb_input,
                                                  ...
                                                  bounds=(-1, 1), T=T)
      if 1:
          true_label = kinetics_classes.index(correct_cls)
          ...
          # target_class = kinetics_classes.index('belly dancing') #javelin throw
          ...
          # criteria = fb_2.criteria.OriginalClassProbability(p=1/500)
...
#%%
    ...
    #     min_, max_ = a.bounds()
    if 1:
      train_tf_record_list=['/media/ROIPO/Data/projects/Adversarial/kinetics-i3d/data/kinetics_triple_jump_train_000.tfrecords'
                            ]
      ...
      while True:
          ...
          x = rgb_sample
          x=x.repeat(_BATCH_SIZE,axis=0)
          label = np.ones(_BATCH_SIZE)*target_class
         ...
         # mask  = rgb_adv_model.session.run(rgb_adv_model._mask)
          ...
          for i in range(0,_SAMPLE_VIDEO_FRAMES, _BATCH_SIZE):
              idx = np.array(range(i,i+_BATCH_SIZE))
