...
def read_npy_file(item):
    ...
    return frames[-_SAMPLE_VIDEO_FRAMES:]

                       
                       
list_train = [x.strip() for x in open('/data/DL/Adversarial/ActivityNet/Crawler/Kinetics/database/train/train_path_list.txt')]
random.shuffle(list_train)
image_paths = tf.convert_to_tensor(list_train, dtype=tf.string)

image_paths = tf.constant(list_train)

dataset = tf.data.Dataset.from_tensor_slices(image_paths)

dataset=dataset.map(
        lambda item: tuple(tf.py_func(read_npy_file, [item], [tf.uint8]) ) ).batch(BATCH_SIZE, drop_remainder=True,num_parallel_calls=20)

dataset.prefetch(BATCH_SIZE*4)
...
ckpt_last = tf.train.latest_checkpoint(checkpoint_dir='model/')
